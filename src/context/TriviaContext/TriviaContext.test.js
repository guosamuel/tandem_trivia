...
const TestComponent = () => {
  ...
  return (
    <>
      ...
      <div data-testid="correct-question-count">
        {state.correctQuestionCount}
      </div>
      <button data-testid="increase-correct-question-count">
        Increase Correct Question Count
      </button>
      <button
        data-testid="choose-random-question"
        onClick={() => dispatch({ type: "CHOOSE_RANDOM_QUESTION", payload: 5 })}
      >
        Choose Random Question
      </button>
      <button
        data-testid="increase-question-count"
        onClick={() => dispatch({ type: "INCREASE_QUESTION_COUNT" })}
      >
        Increase Question Count: {state.questionCount}
      </button>
      <button
        data-testid="start-trivia"
        onClick={() => dispatch({ type: "START_TRIVIA" })}
      >
        Start Trivia: {state.start.toString()}
      </button>
      <button
        data-testid="finish-trivia"
        onClick={() => dispatch({ type: "FINISH_TRIVIA" })}
      >
        Finish Trivia: {(!state.start).toString()}
      </button>
      <button
        data-testid="reset-to-initial-state"
        onClick={() => dispatch({ type: "RESET_TO_INITIAL_STATE" })}
      >
...
it("renders initial state properly", () => {
  ...
  expect(getByTestId("increase-question-count").textContent).toBe(
    "Increase Question Count: 0"
  );
...
it("dispatches actions properly", () => {
  ...
  expect(getByTestId("increase-question-count").textContent).toBe(
    "Increase Question Count: 1"
  );
  userEvent.click(getByTestId("increase-question-count"));
  expect(getByTestId("increase-question-count").textContent).toBe(
    "Increase Question Count: 2"
  );
  userEvent.click(getByTestId("increase-question-count"));
  expect(getByTestId("increase-question-count").textContent).toBe(
    "Increase Question Count: 3"
  );
