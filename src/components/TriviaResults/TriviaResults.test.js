...
it("renders properly with less than 10 questions answered correctly", () => {
  ...
  expect(getByTestId("results-message").textContent).toBe(
    "You answered 7 out of 10 correctly!"
  );
  expect(getByTestId("partial-correct-message").textContent).toBe(
    "There's room for improvement! Let's give it another shot!"
  );
...
it("renders properly with all 10 questions answered correctly", () => {
  ...
  expect(getByTestId("results-message").textContent).toBe(
    "You answered 10 out of 10 correctly!"
  );
  expect(getByTestId("all-correct-message").textContent).toBe(
    "Amazing, you got all 10 questions right!"
  );
  expect(getByTestId("play-again-button").textContent).toBe("Play Again");
});
